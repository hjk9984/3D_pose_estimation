...
def show2Dpose(channels, ax, lcolor="#3498db", rcolor="#e74c3c", add_labels=False):
    ...
    I  = np.array([1,2,3,1,7,8,1, 13,14])-1#,14,18,19,14,26,27])-1 # start points
    J  = np.array([2,3,4,7,8,9,13,14,16])-1#,18,19,20,26,27,28])-1 # end points (#=15)
    LR = np.array([1,1,1,0,0,0,0, 0, 0], dtype=bool)#, 0, 0, 0, 1, 1, 1], dtype=bool)
...
if __name__ == "__main__":
    ...
    with open("test.bin", "rb") as f:
        ...
        stat_3d = torch.load('/Users/antae/Dev_hj/data/human36m/stat_3d.pth.tar')
        print(data[1].shape)
        tmp = unNormalizeData(data[1].numpy()[np.newaxis, ...], stat_3d['mean'], stat_3d['std'], stat_3d['dim_use'])
        tmp = tmp.reshape((-1))
        show3Dpose(tmp.reshape((-1)), plt.subplot(1, 1, 1, projection='3d'))
        # show3Dpose(tmp.reshape((-1)), plt.subplot(gridspec.GridSpec(5,9)[0], projection='3d'))
        plt.show()
        print(tmp)
